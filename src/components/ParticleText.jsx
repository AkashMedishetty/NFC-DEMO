...
const ParticleText = forwardRef(({ words, onAnimationComplete }, ref) => {
  ...
  const nextWord = (word, canvas) => {
    ...
    offscreenCtx.font = 'bold 120px Inter, Arial'
  ...
  const animate = () => {
    ...
    if (isAnimatingRef.current && words && words.length > 0) {
  ...
  useImperativeHandle(ref, () => ({
    start: () => {
      const canvas = canvasRef.current
      if (canvas && words && words.length > 0) {
